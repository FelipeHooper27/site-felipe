  ...
  <div class="container">
    <div class="row">
      <div class="col-6">
        ...
        Sou criativo(a), organizado(a) e atento(a) aos detalhes, garantindo sabor e apresentação. Cozinho com paixão, 
        ...
        <div class="col-4">
          <div class="card" id="id-${index}">
            ...
            <div class="card-footer">
              <button class="btn-open" data-index="${index}" type="button">Confira a galeria</button>
            </div>
          </div>
        </div>
      `).join('')}
  ...
  <div class="modal-overlay">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button class="btn-close" type="button">Fechar</button>
      </div>
    </div>
  </div>
`;
...
function openModal(index) {
  const body = modalFotos.querySelector('.modal-body');
  const album = albuns[index];
  if (!album) return;

  body.innerHTML = `
    <h3>${album.title} — <small>${album.date}</small></h3>
    <p>${album.description}</p>
    <div class="gallery">
      ${album.photos.map(src => `
        <figure>
          <img src="${src}" alt="${album.title}" />
        </figure>
      `).join('')}
    </div>
  `;
...
function closeModal() {
  modalFotos.style.display = 'none';
  modalFotos.querySelector('.modal-body').innerHTML = '';
}

// Delegação de eventos (funciona para elementos já renderizados ou futuros)
document.addEventListener('click', (e) => {
  ...
  // Fechar modal por botão
  if (e.target.matches('.modal-fotos .btn-close')) {
    e.preventDefault();
    closeModal();
    return;
  }

  // Fechar modal clicando fora (na overlay, não no conteúdo)
  if (e.target.classList && e.target.classList.contains('modal-overlay')) {
    closeModal();
  }
});

// Renderização do tema (coloca os filhos dentro de theme content)
